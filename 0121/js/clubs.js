...
getClubDetail: async function(clubId) {
    try {
        ...
        if (response && response.code === 0 && response.data) {
            ...
            console.log('俱乐部详情:', clubDetail);
            
            // 确保 clubDetail 包含 creatorId 信息
            return {
                ...clubDetail,
                // 如果API返回的是 creator，确保有 creatorId 字段
                creatorId: clubDetail.creatorId || null
            };
        } else {
            console.warn('获取俱乐部详情失败:', response);
            return null;
...
setCurrentClub: function(clubId) {
    ...
    // 调试：输出所有俱乐部数据
    console.log('我的俱乐部数据:', this.myClubs);
    ...
    this.myClubs.forEach(club => {
        ...
        // 注意：这里使用 club.memberRole 而不是 club.role
        ...
        if (club.memberRole === 'manager') {
            ...
            actionButtons = `
                <div class="club-actions">
                    ...
                    <button class="btn btn-danger btn-sm" onclick="confirmDeleteClub(${club.id})" title="解散俱乐部">
 ...
 * @param {number} clubId - 俱乐部ID
 */
confirmDeleteClub: function(clubId) {
    // 验证权限
    if (!this.isClubCreator(clubId)) {
        Utils.showNotification('只有俱乐部创建者可以解散俱乐部', 'error');
        return;
    }
    ...
    if (!confirm(`确定要解散俱乐部 "${club.name}" 吗？\n\n解散后将：\n• 所有成员都会被移除\n• 所有任务和视频数据将被删除\n• 此操作不可撤销`)) {
        return;
    }
