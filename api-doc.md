...
- **说明**
  - 若当前俱乐部存在相同内容的软删除记录，则会恢复并复用该记录，不会新建重复记录。
...
- **说明**
  ...
  - 当 `etag` 已存在且当前俱乐部已有该内容记录时：若为未删除记录返回 **409**，若命中软删除记录则自动恢复并复用。
...
- **接口名称**：删除视频
- **接口描述**：删除指定视频。接口会软删除视频记录并写入内容清理任务，由后台异步回收物理内容。
- **接口路径**：`/videos/{id}`
- **请求方法**：DELETE
- **请求头**：`Authorization: Bearer {Token}`
- **认证要求**：登录，且删除者需仍是该俱乐部成员（成员中的上传者或俱乐部管理员可删除）
- **路径参数**：`id`（视频 ID）
- **实现说明**：
  1. 数据库删除流程在同一事务内执行（先对目标 `Contents` 记录加行级写锁，再执行视频软删除并写入 `Jobs` 任务，`jobType=content_cleanup`）。
  2. 接口不再同步删除 `Contents` 与对象存储文件，物理删除由后台异步回收服务处理（含重试）。
- **响应示例（成功）**
...
- **说明**
  - 当当前俱乐部已存在同内容资源时：若为未删除记录返回 **409**，若命中软删除记录则自动恢复并复用。
...
- **说明**
  - 当 `etag` 在全局内容表中不存在时，创建新内容记录；当已存在且当前俱乐部已有该内容记录时：若为未删除记录返回 **409**，若命中软删除记录则自动恢复并复用。
...
- **接口名称**：删除资源
- **接口描述**：删除指定资源。接口会软删除资源记录并写入内容清理任务，由后台异步回收物理内容。
- **接口路径**：`/resources/{id}`
- **请求方法**：DELETE
- **请求头**：`Authorization: Bearer {Token}`
- **认证要求**：登录，且删除者需仍是该俱乐部成员（成员中的上传者或俱乐部管理员可删除）
- **路径参数**：`id`（资源 ID）
- **实现说明**：
  1. 数据库删除流程在同一事务内执行（先对目标 `Contents` 记录加行级写锁，再执行资源软删除并写入 `Jobs` 任务，`jobType=content_cleanup`）。
  2. 接口不再同步删除 `Contents` 与对象存储文件，物理删除由后台异步回收服务处理（含重试）。
- **响应示例（成功）**
...
- **补充说明**：当 `content` 包含 `@{AI用户名}`（其中 AI 用户名来自 `AI_SYSTEM_USER_ID` 对应用户的 `username`）时，后端会自动异步触发 AI 回复任务。
...
- **接口描述**：按视频分页获取评论列表（含发送者信息、当前用户点赞状态、是否 AI 回复）

- **实现说明**：评论列表与当前用户点赞状态（`isLiked`）在同一条列表查询中完成，`isAiReply` 根据发送者是否等于 `AI_SYSTEM_USER_ID` 计算，`total` 由独立计数查询返回。
...
> 注：若原先已点赞，再次调用将返回 `msg: "取消点赞成功"`，`data.liked` 为 `false`。
...
- **接口名称**：触发 AI 回复
- **接口描述**：对指定视频评论手动触发 AI 回复（异步任务，不阻塞当前请求）
- **接口路径**：`/comments/{id}/ai-reply`
- **请求方法**：POST
- **请求头**：`Authorization: Bearer {Token}`
- **认证要求**：登录（仅评论所属俱乐部成员）
- **路径参数**：`id`（评论 ID）
...
```json
{
  ...
  "data": {
    "accepted": true,
    "dedupeKey": "ai-reply:comment:9001"
  }
}
```

> 说明：该接口仅负责入队，AI 回复将由后台 `Jobs` 异步生成并以回复评论形式写入评论区。
...
- **请求体**
  ...
  | subtasks    | array  | 否   | 自定义子任务列表（元素支持 `type/title/description/videoId/pdfId`；如果不传且 type=all，系统会自动生成默认子任务） |

- `isUnlocked` 不传默认 `false`，需手动解锁后才可访问。
- **说明**：当未传 `type` 时，系统会自动生成 3 个子任务：
  1. `read_design` 子任务：标题“阅读教学设计”，描述“阅读并理解本课的教学设计方案”
  2. `watch` 子任务：标题“看视频任务”，描述“观看完整教学视频”
  3. `research` 子任务：标题“研视频任务”，描述“完成教学反思与研讨”
...
1. 若任务未解锁，接口返回 `403`。
2. 若当前子任务类型为 `research`，需先完成同一父任务下其他非 `research` 子任务，才能提交。
3. 非俱乐部成员访问返回 `403`。
...
**说明**：俱乐部成员可访问；若为俱乐部管理员，返回全部任务（包含未解锁任务），否则仅返回已解锁（ `isUnlocked=true`） 的任务；非俱乐部成员访问返回 `403`。软删除任务不会出现在列表中。
...
**说明**：非俱乐部成员访问返回 `403`；若为俱乐部管理员，可查看未解锁任务详情，否则任务未解锁时返回 `403`；已软删除任务返回 `404`。
...
- **接口名称**：修改任务
- **接口描述**：更新任务的主信息（标题、描述、解锁状态），不影响已生成的子任务结构和用户完成记录。
- **接口路径**：`/tasks/{taskId}`
- **请求方法**：PATCH
- **请求头**：`Content-Type: application/json`，`Authorization: Bearer {Token}`
- **认证要求**：登录
...
- **接口名称**：删除任务
- **接口描述**：删除该任务前会先调用话题删除逻辑，删除关联话题；随后软删除任务、子任务 (`Subtasks`) 以及所有用户的完成记录 (`SubtaskCompletions`)。
- **接口路径**：`/tasks/{taskId}`
- **请求方法**：DELETE
- **请求头**：`Authorization: Bearer {Token}`
- **认证要求**：登录
...
| **参数名**       | **类型** | **必填** | **说明** |
| ------------- | ------ | ------ | ------ |
| `type`        | string | 是      | 子任务类型：`watch` / `research` / `read_design` / `lecture` |
| `title`       | string | 是      | 子任务标题 |
| `description` | string | 否      | 子任务描述 |
| `videoId`     | number | 否      | 关联视频 ID，若传入必须属于任务所在俱乐部 |
| `pdfId`       | number | 否      | 关联 PDF 资源 ID，若传入必须属于任务所在俱乐部 |
...
- **接口名称**：修改子任务
- **接口描述**：更新子任务信息（类型、标题、描述、关联视频/PDF）
- **接口路径**：`/tasks/{taskId}/subtasks/{subtaskId}`
- **请求方法**：PATCH
- **请求头**：`Content-Type: application/json`，`Authorization: Bearer {Token}`
- **认证要求**：登录
- **权限**：仅俱乐部管理员（manager）
...
| **参数名**       | **类型** | **必填** | **说明** |
| ------------- | ------ | ------ | ------ |
| `type`        | string | 否      | 子任务类型：`watch` / `research` / `read_design` / `lecture` |
...
- **接口名称**：删除子任务
- **接口描述**：软删除指定子任务，并同步软删除该子任务下所有用户完成记录（`SubtaskCompletions`）
- **接口路径**：`/tasks/{taskId}/subtasks/{subtaskId}`
- **请求方法**：DELETE
- **请求头**：`Authorization: Bearer {Token}`
- **认证要求**：登录
...
- **接口名称**：编辑话题
- **接口描述**：更新话题标题或内容（话题创建者或俱乐部管理员可操作）
- **接口路径**：`/topics/{id}`
- **请求方法**：PATCH
- **请求头**：`Content-Type: application/json`，`Authorization: Bearer {Token}`
- **认证要求**：登录
- **权限**：话题创建者或俱乐部管理员（manager）
- **路径参数**：`id`（话题 ID）
- **请求体**
...
- **接口名称**：删除话题
- **接口描述**：软删除话题及其所有评论（话题创建者或俱乐部管理员可操作）
- **接口路径**：`/topics/{id}`
- **请求方法**：DELETE
- **请求头**：`Authorization: Bearer {Token}`
- **认证要求**：登录
...
- **实现说明**：先聚合校验话题是否存在并获取总评论数，再分页查询评论列表并内联计算当前用户点赞状态（`isLiked`）；响应结构不变。
...
- **接口名称**：编辑评论
- **接口描述**：编辑当前用户在话题下发布的评论内容
- **接口路径**：`/topic-comments/{commentId}`
- **请求方法**：PATCH
- **请求头**：`Content-Type: application/json`，`Authorization: Bearer {Token}`
- **认证要求**：登录
- **权限**：仅评论作者
...
#### 12.1 Agent 对话（流式 SSE）

- **接口名称**：Agent 对话（流式 SSE）
- **接口描述**：调用 Dify Agent，按 SSE 流实时返回增量内容
- **接口路径**：`/ai/agent/chat`
- **请求方法**：POST
- **请求头**：`Content-Type: application/json`，`Authorization: Bearer {Token}`
- **认证要求**：登录
...
| 参数名 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| message | string | 是 | 用户输入消息，最大 10000 字符 |
| conversationId | string | 否 | Dify 会话 ID；续聊时传入 |
| taskId | number | 否 | 任务 ID；传入后后端会校验俱乐部成员权限并注入任务上下文 |
| clubId | number | 否 | 俱乐部 ID；传入后后端会校验俱乐部成员权限并透传给 Agent |

- **响应示例（SSE 片段）**

```text
event: message
data: {"event":"agent_message","answer":"这是第一段"}

event: message
data: {"event":"agent_message","answer":"这是第二段"}

event: done
data: {"event":"message_end","conversation_id":"a4f2..."}
```

- **错误说明**
  - `403`：传入 `taskId` 或 `clubId` 但当前用户不是俱乐部成员
  - `404`：传入 `taskId` 但任务不存在
  - `502/504`：Dify 上游调用失败或超时
...
1. 优化 AI 传入 `taskId` 时注入 Agent 的上下文，增加视频信息及视频评论信息
2. 新增 AI 会话摘要表，简要记录用户与AI的对话信息
3. 评论模块新增手动触发 AI 回复接口：`POST /comments/{id}/ai-reply`（异步入队），，由后台异步生成 AI 回复评论
4. 视频评论列表返回新增 `isAiReply` 字段
5. 发布视频评论时，如果 `content` 包含 `@{AI用户名}` ，会自动触发 AI 回复
...
1. 新增 AI 模块（Dify Agent）接口：`POST /ai/agent/chat`（阻塞式）
2. 支持可选 `taskId` 上下文注入与俱乐部成员权限校验
3. 新增 AI 模块接口文档与错误说明
4. AI 接口 `POST /ai/agent/chat` 从阻塞式改为流式 SSE 返回
...
1. 修改任务和子任务表结构：子任务加一种 type: lecture，并且再加一个PdfID字段，外键关联Resources.ResourceID，可为空。删除父任务的PdfID和VideoID字段。修改相关的业务代码
2. 修改任务删除逻辑，任务/子任务/子任务完成记录 改为软删除
3. 修改子任务的提交逻辑
4. 新增 发布子任务/修改子任务/删除子任务 接口，新增子任务资源归属校验：`videoId` 与 `pdfId` 必须属于任务所在俱乐部
...
1. 新增 编辑话题评论 接口
2. 修改 编辑/删除话题 权限为话题创建者或俱乐部管理员
3. 优化 视频评论/主题评论 的SQL查询
...
1. 视频评论模块新增编辑接口和点赞/取消点赞接口
2. 视频评论列表接口新增返回 `likeCount` 与 `isLiked`
3. 视频评论（Comments）软删除字段由 `IsDeleted` 调整为 `DeletedAt`
4. 话题模块新增删除自己评论接口：`DELETE /topic-comments/{commentId}`
...
1. 修改删除 视频/资源 的权限为俱乐部内成员，且为上传者或俱乐部管理员
2. 修复视频/资源在软删除后再次上传或秒传可能触发唯一索引冲突的问题：命中软删除记录时自动恢复并复用
3. 视频/资源删除 接口的数据库删除流程改为事务执行，避免出现“删除半成功”状态；事务中先对目标 `Contents` 记录加行级写锁，确保同一 contentId 的删除流程串行化
