...
- **错误说明**
  - 当当前俱乐部已存在同内容视频时返回 **409**，提示“视频已存在当前俱乐部视频库中！”
...
注：`sender.realname` 仅当用户 `isPublicProfile = true` 时返回，否则不返回该字段。
...
- **说明**：当未传 `type` 时，系统会自动生成 3 个子任务（继承父任务 videoId）：
  ...
  2. `watch` 子任务：标题“看视频任务”，描述“前往分秒帧平台观看完整教学视频”
  3. `research` 子任务：标题“研视频任务”，描述“前往石墨文档完成教学反思与研讨笔记”
...
- **接口名称**：上报事件
- **接口描述**：上报前端埋点事件数据，支持点击、输入、页面访问、性能等多种事件类型
...
| event_time      | number | 是   | 事件时间（时间戳，毫秒）            |
| user_name       | string | 否   | 用户名                     |
| user_true_name  | string | 否   | 用户真实姓名                  |
| page            | string | 否   | 页面路径                    |
| module          | string | 否   | 模块名称                    |
| event           | string | 是   | 事件类型：`click`/`input`/`change`/`submit`/`page_view`/`route_change`/`scroll_depth`/`page_visible`/`page_hidden`/`js_error`/`resource_error`/`unhandled_rejection`/`page_performance` |
| sub_event       | string | 否   | 子事件类型                   |
| target_object   | string | 否   | 目标对象（如按钮ID、链接URL等）       |
