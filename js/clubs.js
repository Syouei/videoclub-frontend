...
window.Clubs = {
    ...
    container.innerHTML += `
        <div style="grid-column: 1 / -1; margin: 40px 0 20px 0; border-top: 1px solid #eee; padding-top: 20px;">
            ...
            <p style="color: #999; font-size: 13px; margin-top: 4px;">已归档的俱乐部仍然可以查看活动，但不可加入新成员</p>
        </div>
    `;
...
renderClubCard: function(club, container, isArchived) {
    ...
    if (isArchived) {
        // 归档俱乐部：查看任务 + 恢复按钮
        actionButtons = `<button class="btn btn-outline" onclick="goToVideoPage(${club.id})" style="flex:1">
                    <i class="fas fa-eye"></i> 查看活动
                </button>
                <button class="btn btn-primary" onclick="window.Clubs.restoreClub(${club.id})" style="flex:1;white-space: nowrap;">
        ...
        archiveNotice = `<div style="font-size: 12px; color: #d46b08; padding: 6px 8px; background: rgba(250, 173, 20, 0.1); border-radius: 4px; margin-bottom: 12px; display: flex; align-items: center; gap: 6px;">
            ...
            <span>已归档 - 可查看历史活动，不可加入新成员</span>
        </div>`;
    } else if (club.memberRole === 'manager') {
        // 活跃俱乐部创建者：查看任务 + 编辑 + 解散/归档
        actionButtons = `<button class="btn btn-primary" onclick="goToVideoPage(${club.id})" style="flex:1">
        ...
        // 默认：只有查看任务按钮
        actionButtons = `<button class="btn btn-primary" onclick="goToVideoPage(${club.id})" style="width:100%">
                    <i class="fas fa-video"></i> 查看活动
                </button>`;
    }
    ...
    // 修改描述部分，添加 club-description 类
    const descriptionHTML = club.description ? 
        `<div class="club-description" style="font-size:14px; color:#666; margin-bottom:12px; line-height:1.5; flex:1;">${club.description}</div>` : 
        `<div class="club-description" ></div>`;
    
    // 修改入会条件部分
    const joinConditionsHTML = club.joinConditions ? 
    ...
    container.innerHTML += `
        <div class="${cardClass}" style="${cardStyle} display: flex; flex-direction: column; height: 100%; position: relative;">
            ...
            <div style="font-size:13px; color:#999; margin-bottom:16px; display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
                <span><i class="fas fa-hashtag"></i> ID：${club.id}</span>
            ...
            <!-- 操作按钮区域 - 始终在底部 -->
            <div class="club-actions" style="margin-top: auto; padding-top: 16px; border-top: 1px solid rgba(0,0,0,0.05);">
                ${actionButtons}
            </div>
        </div>
    `;
    ...
    quitClub: async function(clubId) {
        if (!confirm('确定要退出这个俱乐部吗？退出后将无法查看俱乐部活动。')) {
            return;
        }
