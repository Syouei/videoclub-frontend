...
window.Profile = {
    ...
    validateProfile: function(profileData) {
        ...
        // 验证学号（当job为student时必填）
        const studentId = profileData.studentId !== undefined && profileData.studentId !== null
            ? profileData.studentId.toString().trim()
            : '';
        if (profileData.job === 'student' && !studentId) {
            errors.push('学生身份需填写学号');
        } else if (studentId) {
            const studentIdRegex = /^[A-Z0-9]{1,20}$/;
            if (!studentIdRegex.test(studentId.toUpperCase())) {
                errors.push('学号格式不正确（1-20位数字/字母）');
            }
        }
    ...
    calculateCompletion: function() {
        ...
        requiredFields.forEach(field => {
            if (this.currentProfile[field] !== undefined && 
                ...
                // 特殊验证
                if (field === 'age') {
                    const age = Number(this.currentProfile[field]);
                    if (Number.isFinite(age) && age >= 0 && age <= 50) {
                        completedCount++;
                    }
                } else if (field === 'phone') {
